...
mqtt_client.on('connect', function(){
	...
	mqtt_client.subscribe('EW/EMETER/#');
});
// MQTT Close Handler
...
mqtt_client.on('message', function (topic, message) {
  ...
  if(topic.toString() == "EW/EMETER/K2177E1E001N001")
  {  
	console.log('Received message on K2177E1E001N001');
	...
	switch (parameter_code)
	{
		...
		case parameterCode.PARAM_NUMBER_USED: // Number used
			...
			if(Number(format_type) == formatType.FORMAT_FLOAT_4BYTE)
			{
				...
				{
					var dataFloat = reverse_buffer(data);        // Dao thu tu chuoi hex
					console.log('---------------------------------> Number used: ',dataFloat.readFloatLE(0));
				}
			}
		break;
